...
Important V57 return-history rule: standalone V37 returns are grouped from their existing `[standalone-return-v37] group=...` `InventoryAdjustment` audit rows. Editing/deleting a return may change only that return's HOME stock contribution and its exact adjustment/movement records; it must not alter sales, Digikala receivable, fees, account entries, payments, or unrelated later stock events.
