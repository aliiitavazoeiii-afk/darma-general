...
BASE=73bb442f5a56c45bc63ddf96fe4bb68ce082b6d6
...
BACKUP="backups/before-daily-color-size-v90-${STAMP}.sql"
...
echo "BACKUP=$BACKUP"
PRE=$(snapshot_exec) || fail "could not capture pre-V90 live state"
echo "$PRE"

step "2) VERIFY V90 SOURCE SCOPE"
git cat-file -e "$BASE^{commit}" || fail "V89 base commit missing"
CHANGED=$(git diff --name-only "$BASE"..HEAD)
echo "$CHANGED"
for f in $CHANGED; do
  case "$f" in
    core/daily_report_v8.py|templates/core/daily_report_v45.html|core/management/commands/check_daily_color_size_v90.py|docs/PROJECT_CONTEXT/42_DAILY_COLOR_SIZE_MATRIX_V90.md|server_daily_color_size_v90.sh) ;;
    *) fail "unexpected V90 file changed: $f" ;;
  esac
done
...
docker compose run --rm --entrypoint sh web -c 'python manage.py collectstatic --noinput >/dev/null && python manage.py check_daily_report_runtime_v48' || fail "daily-report regression failed"
...
PROJECTED=$(snapshot_run) || fail "could not capture V90 projected state"
echo "$PROJECTED"
[ "$PRE" = "$PROJECTED" ] || {
  echo "--- PRE V90 ---"; echo "$PRE"
  echo "--- PROJECTED V90 ---"; echo "$PROJECTED"
  fail "V90 new image changed business state before live recreate"
}
...
docker compose exec -T web python manage.py check_daily_color_size_v90 || fail "live V90 regression failed"

step "6) FINAL BUSINESS INVARIANTS"
FINAL=$(snapshot_exec) || fail "could not capture final V90 state"
echo "$FINAL"
[ "$PRE" = "$FINAL" ] || {
  echo "--- PRE V90 ---"; echo "$PRE"
  echo "--- FINAL V90 ---"; echo "$FINAL"
  fail "V90 changed business/accounting/inventory state"
}

step "7) SUCCESS"
echo "SUCCESS: DAILY COLOR-SIZE MATRIX V90 DEPLOYED"
echo "Daily report: product + physical color rows x size columns"
echo "06 / pack6: aggregated as 06"
echo "Dia Gallery: remains separate"
echo "Backup: $BACKUP"
