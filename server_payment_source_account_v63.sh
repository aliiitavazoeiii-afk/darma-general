...
BASE=9be7c965fd42ad6edb54a4eb39f3494c2f583964
...
print("SELF_PAYMENTS=%d" % BusinessPayment.objects.filter(payee="self").count())
...
' 2>/dev/null | grep -E '^(CAPITAL|MELLAT|MOFID|FINISHED|RAW|DIGI|DIA|TAKVIN_DEBT|DARMA_QTY|TAKVIN_QTY|NOVANI_QTY|SALES|PAYMENTS|SELF_PAYMENTS|RAW_ROWS|MOVEMENTS|TAKVIN_PURCHASES)='
}
...
step "1) DATABASE BACKUP + PRE-MIGRATION SNAPSHOT"
...
BACKUP="backups/before-payment-source-account-v63-${STAMP}.sql"
...
echo "BACKUP=$BACKUP"
PRE=$(snapshot_exec) || fail "could not capture pre-migration business state"
echo "$PRE"

step "2) VERIFY V63 SOURCE SCOPE"
git cat-file -e "$BASE^{commit}" || fail "V63 base commit missing"
CHANGED=$(git diff --name-only "$BASE"..HEAD)
echo "$CHANGED"
for f in $CHANGED; do
  case "$f" in
    core/models.py|core/migrations/0016_businesspayment_source_account.py|core/payment_source_v63.py|core/business_tools_v62.py|core/static/core/payments_source_v63.js|templates/core/payments_v62.html|core/management/commands/check_payment_source_v63.py|server_payment_source_account_v63.sh) ;;
    *) fail "unexpected V63 file changed: $f" ;;
  esac
done

git diff --quiet "$BASE"..HEAD -- \
  core/models_final.py core/urls.py core/report_v10.py core/self_spend_v62.py \
...
POST_MIGRATE=$(snapshot_run) || fail "could not capture post-migration business state"
echo "$POST_MIGRATE"
[ "$PRE" = "$POST_MIGRATE" ] || {
  echo "--- PRE MIGRATION ---"; echo "$PRE"
  echo "--- POST MIGRATION ---"; echo "$POST_MIGRATE"
  fail "V63 migration changed business values"
}
docker compose run --rm --entrypoint python web manage.py shell -c '
from core.models import BusinessPayment
bad=BusinessPayment.objects.exclude(source_account__in=["melat","mofid"]).count()
mofid=BusinessPayment.objects.filter(source_account="mofid").count()
print("INVALID_SOURCE_ROWS=%d" % bad)
print("EXISTING_MOFID_SOURCE_ROWS=%d" % mofid)
assert bad == 0
' || fail "payment source migration verification failed"
...
docker compose run --rm --entrypoint python web manage.py check_payment_source_v63 || fail "V63 regression failed"
...
PROJECTED=$(snapshot_run) || fail "could not capture projected V63 state"
echo "$PROJECTED"
...
docker compose exec -T web python manage.py check_payment_source_v63 || fail "live V63 regression failed"

step "8) FINAL BUSINESS INVARIANTS"
FINAL=$(snapshot_exec) || fail "could not capture final V63 state"
echo "$FINAL"
[ "$PROJECTED" = "$FINAL" ] || {
  echo "--- PROJECTED V63 ---"; echo "$PROJECTED"
  echo "--- FINAL ---"; echo "$FINAL"
  fail "live V63 state differs from projected state"
}

step "9) SUCCESS"
echo "SUCCESS: PAYMENT SOURCE ACCOUNT V63 DEPLOYED"
echo "Payment source selector: ملت / مفید"
echo "Existing payments default to ملت. New payments debit only the selected source account."
echo "Backup: $BACKUP"
