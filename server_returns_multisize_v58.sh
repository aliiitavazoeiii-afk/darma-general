...
BASE=44b2ce63f90df1297d505a547266ce94b69c210b
...
BACKUP="backups/before-returns-multisize-v58-${STAMP}.sql"
...
step "2) VERIFY V58 SOURCE SCOPE"
git cat-file -e "$BASE^{commit}" || fail "V58 base commit missing"
CHANGED=$(git diff --name-only "$BASE"..HEAD)
echo "$CHANGED"
for f in $CHANGED; do
  case "$f" in
    core/returns_v37.py|templates/core/returns_v37.html|core/management/commands/check_returns_multisize_v58.py|server_returns_multisize_v58.sh|docs/PROJECT_CONTEXT/36_RETURNS_MULTI_SIZE_V58.md|docs/PROJECT_CONTEXT/README.md) ;;
    *) fail "unexpected V58 file changed: $f" ;;
  esac
done

git diff --quiet "$BASE"..HEAD -- \
  core/models.py core/models_final.py core/migrations core/final_services.py core/urls.py \
  ...
  || fail "protected non-V58 business source changed"
...
docker compose run --rm --entrypoint python web manage.py check_returns_multisize_v58 || fail "V58 multi-size returns regression failed"
...
[ "$LIVE" = "$PREFLIGHT" ] || {
  ...
  fail "V58 preflight changed persistent business data"
}
...
docker compose exec -T web python manage.py check_returns_history_v57 || fail "live V57 return-history regression failed"
docker compose exec -T web python manage.py check_returns_multisize_v58 || fail "live V58 multi-size returns regression failed"

step "6) VERIFY EXISTING RETURN HISTORY IS STILL READABLE"
docker compose exec -T web python manage.py shell -c '
...
[ "$LIVE" = "$FINAL" ] || {
  ...
  fail "V58 deploy changed business data; deployment itself must be read-only"
}

step "8) SUCCESS"
echo "SUCCESS: RETURNS MULTI-SIZE V58 DEPLOYED"
echo "Backup: $BACKUP"
echo "New return entry: one date + all sizes on one page + one atomic submit."
echo "Existing V57 history/view/edit/delete remains intact per size report."
