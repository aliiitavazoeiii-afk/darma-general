...
BASE=4695bb45c685c300e9695dbb12f1ee735462ecb2
SECRET_ROOT=/opt/darma-secrets/digikala
RUNTIME_SECRET="$SECRET_ROOT/runtime"

step "1) VERIFY DIGIKALA RUNTIME SECRETS"
[ -d "$RUNTIME_SECRET" ] || fail "V40 runtime secret directory missing: $RUNTIME_SECRET"
...
BACKUP="backups/before-digikala-deliveries-v41-${STAMP}.sql"
...
step "5) V41 SOURCE-SCOPE GUARD"
git cat-file -e "$BASE^{commit}" || fail "pre-v41 base missing"
CHANGED=$(git diff --name-only "$BASE"..HEAD)
echo "$CHANGED"
for f in $CHANGED; do
  case "$f" in
    core/digikala_delivery_v41.py|core/digikala_views_v40.py|core/management/commands/check_digikala_delivery_v41.py|templates/core/digikala_v40.html|templates/core/dashboard_excel.html|server_digikala_deliveries_v41.sh|UI_SAFETY_V41.md|docs/00_NEW_CHAT_READ_FIRST.md|docs/PROJECT_CONTEXT/README.md|docs/PROJECT_CONTEXT/03_ACTIVE_CODE_MAP.md|docs/PROJECT_CONTEXT/08_LIVE_STATE_AND_CHECKPOINTS.md|docs/PROJECT_CONTEXT/18_DIGIKALA_API_V40.md|docs/PROJECT_CONTEXT/19_DIGIKALA_DELIVERIES_V41.md) ;;
    *) fail "unexpected V41 file changed: $f" ;;
  esac
done

git diff --quiet "$BASE"..HEAD -- core/finance.py core/report_v9.py core/inventory_valuation_v17.py core/business_tools_v22.py core/material_report_v22.py core/final_services.py core/cost_accounting_v14.py core/finance_excel_v9.py core/daily_order_import_v23.py core/returns_v37.py core/calculator_v37.py core/models.py core/migrations core/urls.py compose.yml || fail "protected business/route/secret-mount source changed in V41"
...
delivery=Path("/app/core/digikala_delivery_v41.py").read_text(encoding="utf-8")
...
page=Path("/app/templates/core/digikala_v40.html").read_text(encoding="utf-8")
assert "effectiveCommitments" in delivery and "nextDays" in delivery
assert "get_delivery_board" in views
assert "dkDeliveries" in dash and "delivery_effective_total" in dash
assert "ریز کالاهایی که باید تحویل بدهی" in page
...
print("V41 SOURCE / READ-ONLY / SECRET SAFETY CHECK OK")
' || fail "V41 source safety failed"

step "8) VERIFY PREFLIGHT CHANGED NO BUSINESS VALUES"
NEW=$(snapshot_new) || fail "could not read new-image business values"
echo "$NEW"
[ "$LIVE" = "$NEW" ] || fail "business values changed during V41 preflight"
...
echo "$FINAL"
[ "$LIVE" = "$FINAL" ] || fail "V41 read-only delivery UI changed business values"

echo ""
echo "======================================"
echo "SUCCESS: DIGIKALA DELIVERIES V41 DEPLOYED"
echo "Backup: $BACKUP"
echo "Dashboard: effective Digikala commitments replace misleading current-order count"
echo "Digikala page: read-only item-level delivery list with code/size/title/quantity/search"
