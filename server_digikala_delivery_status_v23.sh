...
mkdir -p backups
BACKUP="backups/before-digikala-delivery-status-v23-$(date +%Y%m%d-%H%M%S).sql"
...
docker compose run --rm --entrypoint python web manage.py check_v23_delivery_import || fail "v23 delivery status check failed"
...
docker compose exec -T web python manage.py check_v23_delivery_import || fail "live v23 delivery check failed"
...
echo ""
echo "======================================"
echo "SUCCESS: DIGIKALA DELIVERY STATUS V23 DEPLOYED"
echo "Backup: $BACKUP"
echo "Capital unchanged by deployment: $CAP_AFTER"
echo "Darma stock unchanged by deployment: $DARMA_AFTER"
echo "Accepted current status: اماده ارسال/تحویل"
echo "Return/cancel/failed statuses remain blocked."
echo "======================================"
