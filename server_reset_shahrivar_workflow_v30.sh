...
# Only remove disposable `docker compose run` containers. Do not touch the telegram bot orphan.
...
BACKUP="backups/before-shahrivar-workflow-reset-v30-${STAMP}.sql"
PLAN="backups/plan-shahrivar-workflow-reset-v30-${STAMP}.txt"
APPLYLOG="backups/apply-shahrivar-workflow-reset-v30-${STAMP}.txt"
...
docker compose run --rm --entrypoint python web manage.py reset_shahrivar_workflow_v30 2>&1 | tee "$PLAN" || fail "reset plan failed"
grep -q "READ ONLY" "$PLAN" || fail "plan did not finish as read-only"
echo "PLAN = $PLAN"

step "6) APPLY ATOMIC RESET FROM 1405/06/01"
docker compose run --rm --entrypoint python web manage.py reset_shahrivar_workflow_v30 --apply 2>&1 | tee "$APPLYLOG" || fail "reset failed; transaction should be rolled back"
...
grep -q "Payments from 1 Shahrivar onward = 0" "$APPLYLOG" || fail "payment verification failed"
...
echo ""
echo "======================================"
echo "SUCCESS: SHAHRIVAR WORKFLOW RESET V30"
echo "Backup: $BACKUP"
echo "Plan: $PLAN"
echo "Apply log: $APPLYLOG"
echo "All SaleDays/SaleLines from 1 Shahrivar onward: REMOVED"
echo "All outgoing payments from 1 Shahrivar onward: REVERSED + REMOVED"
...
echo "Next: enter ONLY 1, 2, 3 Shahrivar; then restore/check exact physical day-3 inventory baseline."
echo "======================================"