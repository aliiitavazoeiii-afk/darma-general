...
mkdir -p backups
BACKUP="backups/before-daily-edit-prepayment-v21-$(date +%Y%m%d-%H%M%S).sql"
...
step "6) VERIFY CODE DEPLOYMENT DID NOT TOUCH DATA"
...
docker compose exec -T web python manage.py check_v21_workflows || fail "live v21 check failed"
...
[ "$CAP_BEFORE" = "$CAP_AFTER" ] || fail "capital changed by deployment: before=$CAP_BEFORE after=$CAP_AFTER"
[ "$DARMA_BEFORE" = "$DARMA_AFTER" ] || fail "Darma stock changed by deployment: before=$DARMA_BEFORE after=$DARMA_AFTER"

echo ""
echo "======================================"
echo "SUCCESS: DAILY EDIT + PREPAYMENT + CASHFLOW V21 DEPLOYED"
echo "Backup: $BACKUP"
