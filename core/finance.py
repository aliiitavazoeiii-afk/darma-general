...
def sale_line_metrics(line):
    ...
    pack_qty = int(line.product_size.product.pack_qty or 0)
    gross = qty * int(line.sale_price or 0)
    digikala_fee = qty * digikala_fee_for_unit(line.sale_price)
    shorts = qty * pack_qty
    cogs = shorts * int(line.product_size.unit_cost or 0)
    profit = gross - digikala_fee - cogs
    margin = (profit / gross * 100) if gross else 0
    return {
        "gross": gross,
        "digikala_fee": digikala_fee,
        "cogs": cogs,
        "profit": profit,
        "margin": margin,
        "shorts": shorts,
