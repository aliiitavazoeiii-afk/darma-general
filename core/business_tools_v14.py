...
def _find_row(section, needle, create=False, title=None):
    ...
    for row in qs:
        if needle in (row.title or "").replace(" ", "").lower():
            return row
    if not create:
        return None
...
def _digikala_account():
    obj, _ = Account.objects.get_or_create(
        key=Account.DIGIKALA, defaults={"title": "دیجی‌کالا", "opening_balance": 0}
    )
    return obj
    ...
    try:
        ...
        with transaction.atomic():
            ...
            payment = BusinessPayment.objects.create(
                ...
                payee=payee,
                amount=amount,
                # keep a compact compatibility copy, but v14 uses MoneyMovement ledger as source of truth
        ...
        if purchase_data:
            messages.success(request, "خرید مواد ثبت شد؛ پول ملت و موجودی مواد در یک تراکنش دوطرفه ثبت شدند.")
        else:
    ...
    try:
        with transaction.atomic():
            ...
            if payment.payee in MATERIAL_PAYEES:
                ...
                if not data:
                    raise ValueError(
                        "این پرداخت مربوط به مواد اولیه است ولی Ledger خرید آن پیدا نشد؛ برای جلوگیری از افزایش مصنوعی سرمایه حذف متوقف شد."
                    )
                ...
                if ledger:
                    ledger.delete()
                payment.delete()
                messages.success(request, "خرید حذف شد؛ همان مواد از انبار کم و همان پول به ملت برگشت.")
                return redirect("/payments/?section=payments")
...
def receipt_add(request):
    try:
        ...
        messages.success(request, "دریافتی ثبت شد؛ طلب دیجی کم و ملت به همان مبلغ زیاد شد. سرمایه نباید تغییر کند.")
    except Exception as exc:
        messages.error(request, str(exc))
    ...
    try:
        with transaction.atomic():
            ...
            deleted, _ = AccountEntry.objects.filter(
                account=account, reference=f"receipt:{receipt.id}:digikala", entry_type="receipt"
            ).delete()
            if not deleted:
                raise ValueError("Ledger طلب دیجی برای این دریافتی پیدا نشد؛ حذف متوقف شد تا سرمایه یک‌طرفه تغییر نکند.")
            receipt.delete()
